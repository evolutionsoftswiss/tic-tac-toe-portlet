...
public class Playground {
	...
	public boolean hasWon(char player) {
		...
		return this.horizontalWin(player)
	...
	private boolean diagonalWin(char player) {
		...
		return (this.position[0][0] == player
		     && this.position[1][1] == player
			 && this.position[2][2] == player)
			|| (this.position[2][0] == player
			 && this.position[1][1] == player
			 && this.position[0][2] == player);
	}

	private boolean verticalWin(char player) {

		return (this.position[0][0] == player
			 && this.position[1][0] == player
		     && this.position[2][0] == player)
			|| (this.position[0][1] == player
			 && this.position[1][1] == player
		     && this.position[2][1] == player)
	        || (this.position[0][2] == player
		     && this.position[1][2] == player
			 && this.position[2][2] == player);
	}
